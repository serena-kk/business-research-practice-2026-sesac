...
가장 단순하고 직관적인 방식. Likert 5점 척도(1~5)를 그대로 쓰고, 중간값 **3.0**을 중심선으로 삼는다.
...
이미 사업 운영 중에 있고 여러 차례 도출된 점수 분포가 존재하는 경우, 평균값과 표준편차를 이용해 사분면을 **데이터 기반으로 자동 구분**할 수 있다.
...
> 💡 **변형: 평균(μ) 대신 중앙값(median) 사용**
>
> 표본 수가 적거나(예: 페르소나 10여 개), 배영자(Non-user)처럼 Importance·Satisfaction이 극단값(1점대/5점대)을 가진 항목이 섞여 있으면 평균이 극단값 쪽으로 쉽게 끌려간다. 이런 경우 평균 대신 **중앙값**을 기준선으로 쓰면 극단값의 영향을 줄이고, "전체 항목의 절반은 상단/우측, 절반은 하단/좌측"이라는 명확한 기준으로 사분면을 나눌 수 있다.
>
> | 항목 | 계산 | 해석 |
> | --- | --- | --- |
> | Importance 기준선 | 전체 Importance의 중앙값(median) | 중앙값 이상 → 상단 |
> | Satisfaction 기준선 | 전체 Satisfaction의 중앙값(median) | 중앙값 이상 → 우측(High Satisfaction) |
...
## 3. AOS에서 시장 가중형 점수 DOS로 확장하기 – VC들이 실제로 사용하는 시장가중형 분석

> AOS는 고객 한 명의 중요도를 반영한 지표라면,
> DOS는 **시장 규모와 맥락을 곱해 '발견된 기회(Discovered Opportunity Score)'를 산출**한다.
>
> → **기회 점수 = 고객 미충족 × 시장 파급력** 으로 계산한다. (실제 VC, PM들이 쓰는 구조와 유사)
...
> ```
>
> → **Market Relevance 수치는 시장 파급력**을 뜻하며, `TAM-SAM-SOM(%)`*을 사용하거나, 시장 성장률, 채택난이도, 확산성 등을 추가로 고려해 평가한다.
>
> *TAM-SAM-SOM 중 적정 모수에 대해 해당 Pain/Goal이 갖는 상대적 비중.
>
> ⇒ 지금까지 수행한 시장분석 자료를 AI에 투입하면 "기회 강도 × 시장 확산성"을 손쉽고 정확하게 도출할 수 있다.
